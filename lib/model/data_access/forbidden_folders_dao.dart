...
class ForbFolderDAO implements DAOInterface<ForbFolder, int> {
  ...
  Future<bool> delete(ForbFolder item) async {
    try {
      var res = await SQLiteUtils.db
          .delete('forbFolders', where: 'id=?', whereArgs: [item.id]);
      return res == 1;
    } catch (e) {
      Logger().e(e);
      return false;
  ...
  Future<ForbFolder?> get(int value) async {
    try {
      var res = await SQLiteUtils.db
          .query('forbFolders', where: 'id=?', whereArgs: [value]);
      var line = res.first;
      return ForbFolder(
  ...
  Future<bool> insert(ForbFolder item) async {
    try {
      var res = await SQLiteUtils.db
          .insert('forbFolders', {'name': item.name, 'route': item.route});
      return res == 1;
    } catch (e) {
      Logger().e(e);
      return false;
  ...
  Future<bool> update(ForbFolder item) async {
    try {
      var res = await SQLiteUtils.db.update(
          'forbFolders', {'name': item.name, 'route': item.route},
